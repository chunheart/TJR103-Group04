...
### CSV ###
CSV_FILE_PATH = ROOT_DIR / "data" / "db_ingredients" / f"icook_recipe_{MANUAL_DATE}_recipe_ingredients_unitN.csv"
MOVE_DIR = ROOT_DIR / "data" / "db_ingredients" / "ingredient_unit_normalization" / f"processed_ingredient_unit={MANUAL_DATE}"
MOVE_DIR.mkdir(exist_ok=True, parents=True)
MOVE_PATH = MOVE_DIR / f"icook_recipe_{MANUAL_DATE}_recipe_ingredients_unitN.csv"


def mysql_unit_normalization():
    """
    Goal:
        ...
        try:
            ...
            try:
                ...
                INSERT INTO `{TABLE_NAME}`
                    (`ori_ingredient_name`, `unit_name`, `weight_grams`, `ins_time`, `upd_time`, `u2g_status`)
                VALUES
                    (%(ingredients)s, %(unit_name)s, %(Normalized_Weight_g)s, DEFAULT, DEFAULT, 'done');
                """
                ...
                # os.rename(src=CSV_FILE_PATH, dst=MOVE_PATH)
