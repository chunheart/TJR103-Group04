...
SAVED_FILE_PATH =  SAVED_FILE_DIR / f"icook_recipe_{COLLECTION}.csv"
...
def main():
    ...
    """
    column = ["ori_ingredient_name", "nor_ingredient_name"]
    ori_list = []
    nor_list = []
    for ori, nor in INGREDIENT_MAP.items():
        ori_list.append(ori)
        nor_list.append(nor)
    
    data = {
        "ori_ingredient_name": ori_list,
        "nor_ingredient_name": nor_list,
    }
        
    df = pd.DataFrame(data)
    int_time, upd_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S"), datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    status = "pending"
    df["ins_timestamp"] = int_time
    df["upd_timestamp"] = upd_time
    df["status"] = status
