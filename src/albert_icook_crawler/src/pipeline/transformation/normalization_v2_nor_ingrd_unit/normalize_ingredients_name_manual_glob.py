...
def process_ingredients_pipeline(dt:pd.DataFrame, ref_path:Path, output_file_path:Path):
    """
    ...
    if unique_unknowns:
        ...
        BATCH_SIZE = 200 # Process 100 ingredients at a time
    ...
    # Save to disk
    target_file = str(output_file_path).split("/")[-1].split(".")[0] # icook_7_recipe_ingredients
    output_norm_ingred_name_file = ROOT_DIR / "data" / "db_ingredients" / f"{target_file}_name_norm.csv" # icook_7_recipe_ingredients_name_norm.csv
...
def normalize_ingrd_name():
    ...
    try:
        file_pattern = ROOT_DIR / "data" / "db_ingredients" / "icook_34*_recipe_ingredients.csv"
