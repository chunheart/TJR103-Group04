...
ROOT_DIR = Path(__file__).resolve().parents[3] # root is dir src
ENV_PATH = Path(ROOT_DIR/"src"/"utils"/".env")
load_dotenv(ENV_PATH)

FILENAME = os.path.basename(__file__).split(".")[0]
LOG_FILE_DIR = ROOT_DIR / "logs" / f"logs={datetime.today().date}"
...
SAVED_FILE_PATH =  SAVED_FILE_DIR / f"icook_recipe_2025-11-19_{DATABASE}_{COLLECTION}.csv"
...
def main():
    ...
    try:
        ...
        ingredient_df_explode["unit_name"] = ingredient_df_explode["unit_name"].apply(unit_g_convertion)
