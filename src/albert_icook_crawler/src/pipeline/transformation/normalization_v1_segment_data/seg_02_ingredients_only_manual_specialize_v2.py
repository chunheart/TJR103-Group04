...
def float_conversion(value):
    try:
        ...
        logger.error(f"{value} has an issue: {e}")
...
def get_ingredients_info():
    ...
    try:
        ...
        df_to_process = get_rows_needing_processing(raw_df)
        ...
        if not df_to_process.empty:
            ...
            if all_predictions:    
                df_final, count = update_dataset(raw_df, all_predictions)
        ...
        try:
            df_final["unit_number"] = df_final["unit_number"].apply(float_conversion)
        except ValueError:
            pass
