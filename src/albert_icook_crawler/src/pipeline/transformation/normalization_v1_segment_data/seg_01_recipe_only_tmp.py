...
CSV_FILE_PATH = CSV_ROOT_DIR / "data" / "processed" /f"processed=2025-11-16" / f"icook_recipe_2025-11-16.csv"

def main():
    """
    Retrieve icook recipe CSV file from the determined field to MongoDB, collection is recipe
    ...
    try:
        ...
        # print(duplicates_removal_df)

        ### Transform
        logger.info("Filling None values with \"1人份\" in field people ...")
        # Fill None with "1人份"
        duplicates_removal_df["people"] = duplicates_removal_df["people"].fillna("1人份", inplace=False)
        logger.info("Filled None values with \"1人份\" in field people")

        # Convert number into integer type
        logger.info("Converting values' data type in field people into integer ...")
        duplicates_removal_df["people"] = duplicates_removal_df["people"].apply(ppl.get_recipe_ppl_num)
        logger.info("Converted values' data type in field people into integer ...")
        duplicates_removal_df.info()
